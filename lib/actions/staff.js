...
export async function acceptStaffInvite(token, fullName, password) {
  ...
  // use admin API — bypasses email confirmation and rate limits
  ...
  console.log("2. Auth data:", JSON.stringify(authData))
...
export async function assignVendorToEvent(eventId, vendorId, notes) {
  const supabase = await createClient()
  ...
  if (existing) return { error: "Vendor is already assigned to this event" }
...
export async function updateVendorStatus(eventVendorId, status) {
  const supabase = await createClient()
...
export async function removeVendorFromEvent(eventVendorId) {
  const supabase = await createClient()
